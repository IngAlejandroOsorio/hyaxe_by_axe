...
mp.events.add("playerEnterColshape", (player, colshape) => {
    ...
    player.call("tomarDinero", [colshape.cashAmount]);
    setTimeout(() => {     
            if (colshape.timer) {
...
mp.events.add("cashRegisters::shot", (player, cashRegisterId, dropPos) => {
    ...
    //setTimeout(() => {hitEntity.model = mp.game.joaat('prop_till_01')},300000);
    cashRegister.setState(cashRegisterState.shot);

    // Create cash "pickup"
    var cashArea = mp.colshapes.newSphere(dropPos.x, dropPos.y, dropPos.z, 1.0);
    cashArea.cashAmount = getRandomInt(50, 150);
