...
namespace DowntownRP.Utilities
{
    public class Generate
    {
        ...
        public async static Task<string> GenerateDNI()
        {
            string matricula = CreateDNI();
            if (await CheckIfDNIExists(matricula)) matricula = CreateDNI();
            return matricula;
        }

        public async static Task<bool> CheckIfDNIExists(string numberplate)
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT * vehicles_companies WHERE dni = @dni";
                command.Parameters.AddWithValue("@dni", numberplate);
        ...
        public static string CreateDNI()
        {
