...
namespace DowntownRP.World.Banks
{
    ...
    {
        public async static void SpawnBank()
        {
                ...
                if (reader.HasRows)
                {
                    while (await reader.ReadAsync().ConfigureAwait(false))
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            if (type == 1)
                            {
                                blip = NAPI.Blip.CreateBlip(108, position, 1, 5, "Banco", 255, 0, true);
                            }
                            else
                            {
                                blip = NAPI.Blip.CreateBlip(277, position, 1, 5, "ATM", 255, 0, true);
                            }
