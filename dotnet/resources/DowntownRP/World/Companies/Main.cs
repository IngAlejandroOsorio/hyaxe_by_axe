...
namespace DowntownRP.World.Companies
{
    public class Main : Script
    {
        ...
        public async Task Banks_EnterColShape(ColShape shape, Player player)
        {
            if (shape.HasData("COMPANY_ID"))
            {
                player.SetData("COMPANY_ID", shape.GetData<int>("COMPANY_ID"));
            }

            if (player.HasData("COMPANY_RACE"))
            {
                Data.Entities.Race race = player.GetData<Data.Entities.Race>("COMPANY_RACE");
                if (shape == race.shape)
                {
                    if(race.step == 21)
                    {
                        player.TriggerEvent("chat_goal", "¡Felicidades!", $"Has ganado ${race.ganancies} por el recorrido");
                        await Game.Money.MoneyModel.AddMoney(player, race.ganancies);
                        player.WarpOutOfVehicle();
                        player.ResetData("COMPANY_RACE");
                        player.TriggerEvent("TruckerDestroyBlip");
                        player.TriggerEvent("stop_chat_notification");
                    }
                }
            }
            ...
            if (shape.HasData("BUSINESS_CLASS"))
            {
                ...
                {
                    Data.Entities.Race race = player.GetData<Data.Entities.Race>("COMPANY_RACE");
                    if(business.id == race.businessId)
                    {
                        if(race.step == 10)
                        {
                            race.step = 11;
                            player.TriggerEvent("chat_notification", "~r~Descarga las cajas~w~ del camión presionando N");
                            return;
                        }
                    }
                }
            ...
            if (shape.HasData("COMPANY_DUTY"))
            {
                if (shape.GetData<int>("COMPANY_DUTY") == user.job)
                {
                    user.isInCompanyDuty = true;
                    if (user.companyMember.type == 4) player.TriggerEvent("tipSellMinerales");
                }
            }
        ...
        [RemoteEvent("ActionCompanyDuty")]
        public void RE_ActionCompanyDuty(Player player)
        {
            //var user = player.GetExternalData<Data.Entities.User>(0);
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData<Data.Entities.User>("USER_CLASS");
            if (user.isInCompanyDuty)
            {
