...
namespace DowntownRP.World.Companies
{
    public class Main : Script
    {
        ...
        public async Task Banks_EnterColShape(ColShape shape, Client player)
        {
            if (shape.HasData("COMPANY_ID"))
            {
                player.SetData("COMPANY_ID", shape.GetData("COMPANY_ID"));
            }
            ...
            {
                if (shape.GetData("COMPANY_DUTY") == user.job)
                {
        ...
        public void RE_ActionCompanyDuty(Client player)
        {
            //var user = player.GetExternalData<Data.Entities.User>(0);
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData("USER_CLASS");
        ...
        public async Task RE_SignContractCompany(Client player)
        {
            ...
            if (user.isInCompanyContract)
            {
                ...
                await Axe.Creador.Main.UpdateUserCompany(user.idpj, user.companyInterior.id);
