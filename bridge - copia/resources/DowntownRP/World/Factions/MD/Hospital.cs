...
namespace DowntownRP.World.Factions.MD
{
    public class Hospital : Script
    {
        ...
        public void SE_HospitalEnterColshape(ColShape shape, Client player)
        {
        ...
        public void SE_HospitalExitColshape(ColShape shape, Client player)
        {
            if (shape.HasData("HOSPITAL_LOBBY"))
            {
        ...
        public void CMD_comprarseguro(Client player)
        {
        ...
        public async Task RS_BuySeguroMedico(Client player)
        {
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData("USER_CLASS");
            ...
            if (player.HasData("HOSPITAL_LOBBY"))
            {
                if (!user.seguroMedico)
                {
                    if(await Game.Money.MoneyModel.SubMoney(player, 1000))
                    {
                        ...
                        await Axe.Creador.Main.UpdateSeguroMedico(user.idpj, 1);
