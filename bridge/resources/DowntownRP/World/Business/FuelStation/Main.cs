...
namespace DowntownRP.World.Business.FuelStation
{
    public class Main : Script
    {
        ...
        public void SE_EnterColShapeFuel(ColShape shape, Client player)
        {
            if (!shape.HasData("BUSINESS_FUEL_POINT")) return;
            player.SetData("BUSINESS_FUEL_POINT", shape.GetData("BUSINESS_FUEL_POINT"));
        }

        [ServerEvent(Event.PlayerExitColshape)]
        public void SE_ExitColShapeFuel(ColShape shape, Client player)
        {
            if (!shape.HasData("BUSINESS_FUEL_POINT")) return;
        ...
        public void RE_ActionFuelGas(Client player)
        {
            if (player.HasData("BUSINESS_FUEL_POINT"))
            {
                if (player.IsInVehicle)
                {
                    ...
                    {
                        // 1 litro = $2
                        int fuel = player.Vehicle.GetSharedData("FUEL");
        ...
        public async Task RE_PlayerPayFuel(Client player)
        {
            if (player.IsInVehicle)
            {
                int fuel; int money;
                fuel = 100 - player.Vehicle.GetSharedData("FUEL"); money = (100 - player.Vehicle.GetSharedData("FUEL")) * 2;
                if (fuel + player.Vehicle.GetSharedData("FUEL") <= 100)
                {
                    if (await Game.Money.MoneyModel.SubMoney(player, money) == false) Utilities.Notifications.SendNotificationERROR(player, "No tienes suficiente dinedddro");
                    else
                    {
                        player.Vehicle.SetSharedData("FUEL", player.Vehicle.GetSharedData("FUEL") + fuel);
