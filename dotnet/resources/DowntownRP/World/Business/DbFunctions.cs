...
namespace DowntownRP.World.Business
{
    ...
    {
        public static async void SpawnBusiness()
        {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        NAPI.Task.Run(async () =>
                        {
                            ...
                            Data.Lists.business.Add(business);
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            if (isRentSelling)
                            {
                                 label = NAPI.TextLabel.CreateTextLabel($"~y~{type}~n~~w~Precio Renta: ~g~${price} x Minuto", position, 8, 1, 0, new Color(255, 255, 255));
                            }
                            else
                            {
                                label = NAPI.TextLabel.CreateTextLabel($"~y~{type}~n~~w~Precio: ~g~${price}", position, 8, 1, 0, new Color(255, 255, 255));
                            }                        
        ...
        public async static Task CreateFuelPoint(Player player, int business)
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "INSERT INTO fuel_points (business, x, y, z) VALUES (@business, @x, @y, @z)";
                command.Parameters.AddWithValue("@business", business);
                command.Parameters.AddWithValue("@x", player.Position.X);
                command.Parameters.AddWithValue("@y", player.Position.Y);
                command.Parameters.AddWithValue("@z", player.Position.Z);
        ...
        public static async Task SpawnFuelPoints(Data.Entities.Business business)
        {
            try
            {
                using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
                {
                    await connection.OpenAsync().ConfigureAwait(false);
                    MySqlCommand command = connection.CreateCommand();
                    command.CommandText = "SELECT * FROM fuel_points WHERE business = @business";
                    command.Parameters.AddWithValue("@business", business.id);
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            NAPI.Task.Run(() =>
                            {
                                ColShape gaspoint = NAPI.ColShape.CreateCylinderColShape(position, 2, 2);
                                //NAPI.Marker.CreateMarker(27, position.Subtract(new Vector3(0, 0, 1)), new Vector3(), new Vector3(), 2, new Color(255, 151, 0));
                                NAPI.TextLabel.CreateTextLabel("Presiona ~o~F5 ~w~para poner gasolina", position.Subtract(new Vector3(0, 0, 0.1)), 4, 2, 2, new Color(255, 255, 255));
