...
namespace DowntownRP.World.Companies
{
    ...
    {
        public async static void SpawnCompanies()
        {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            NAPI.Task.Run(async () =>
                            {
                                ...
                                shape_duty.SetData("COMPANY_DUTY", dcompany.id);


                                Data.Lists.Companies.Add(dcompany);
                                await SpawnVehicleCompanies(dcompany);
                                await SpawnTruckerStartCompany(dcompany);
                            });
        ...
        public async static Task<int> CreateCompanyVehicle(int company, string model, int color1, int color2, string numberplate, double x, double y, double z, double rot)
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "INSERT INTO vehicles_companies (company, model, color1, color2, numberplate, x, y, z, rot) VALUES (@company, @model, @color1, @color2, @numberplate, @x, @y, @z, @rot)";
        ...
        public static async Task SpawnTruckerStartCompany(Data.Entities.Company company)
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT * FROM truckers_start WHERE company = @company";
                command.Parameters.AddWithValue("@company", company.id);
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        NAPI.Task.Run(() =>
                        {
                            ...
                            shape.SetData("COMPANY_ID", company.id);
                        });
                    }
                }
            }
        ...
        public async static Task<int> CreateTruckerStartCompany(int company, double x, double y, double z)
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "INSERT INTO truckers_start (company, x, y, z) VALUES (@company, @x, @y, @z)";
                command.Parameters.AddWithValue("@company", company);
                command.Parameters.AddWithValue("@x", x);
                command.Parameters.AddWithValue("@y", y);
                command.Parameters.AddWithValue("@z", z);
        ...
        public static async Task<List<double>> GetVector3OfSupermarket()
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT * FROM business WHERE type = 1 ORDER BY RAND() LIMIT 1";
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        return new List<double>() { id, x, y, z };
                    }
                }
                return null;
            }
        ...
        public static async Task<Vector3> GetTruckerStart(int idcompany)
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT * FROM truckers_start WHERE company = @company ORDER BY RAND() LIMIT 1";
                command.Parameters.AddWithValue("@company", idcompany);
        ...
        public static async Task SpawnMinerPoints()
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT * FROM miner_points";
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        NAPI.Task.Run(async () =>
                        {
                            ColShape shape = NAPI.ColShape.CreateCylinderColShape(position, 1, 1);
                            GTANetworkAPI.Object obj = NAPI.Object.CreateObject(-1625949270, position.Subtract(new Vector3(0, 0, 1.2)), new Vector3(0, 0, rot));
                            TextLabel label = NAPI.TextLabel.CreateTextLabel("Pulsa ~g~E~w~ para picar piedra~n~Recursos: ~g~25/25", position, 2, 1, 0, new Color(255, 255, 255));
        ...
        public async static Task<int> CreateMinerPoint(double x, double y, double z, double rot)
        {
            using (MySqlConnection connection = new MySqlConnection(Data.DatabaseHandler.connectionHandle))
            {
                await connection.OpenAsync().ConfigureAwait(false);
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "INSERT INTO miner_points (x, y, z, rot) VALUES (@x, @y, @z, @rot)";
                command.Parameters.AddWithValue("@x", x);
                command.Parameters.AddWithValue("@y", y);
                command.Parameters.AddWithValue("@z", z);
                command.Parameters.AddWithValue("@rot", rot);
