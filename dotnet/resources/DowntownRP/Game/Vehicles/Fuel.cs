...
namespace DowntownRP.Game.Vehicles
{
    public class Fuel : Script
    {
        ...
        public async Task Fuel_PlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seatID)
        {
            ...
            if(seatID == -1) // Cambiar en la 1.1 a 0
            {
                while (player.IsInVehicle)
                {
                    ...
                    {
                        int actualFuel = vehicle.GetSharedData<int>("FUEL");
                        if (actualFuel <= 0)
                        {
        ...
        [Command("checkfuel")]
        public void CMD_checkfuel(Player player)
        {
            if (player.IsInVehicle)
            {
                player.SendChatMessage($"La gasolina del vehículo es {player.Vehicle.GetSharedData<int>("FUEL")}");
            }
        }
    }
}
