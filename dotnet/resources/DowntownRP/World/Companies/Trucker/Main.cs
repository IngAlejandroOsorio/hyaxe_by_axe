...
namespace DowntownRP.World.Companies.Trucker
{
    public class Main : Script
    {
        ...
        public async Task RE_TruckerStartRace(Player player, int type)
        {
        ...
        public async Task RE_ActionCargueroInteract(Player player)
        {
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData<Data.Entities.User>("USER_CLASS");
            ...
            if (user.companyMember.id == player.GetData<int>("COMPANY_ID"))
            {
                if (race.step == 0)
                {
        ...
        public void RE_BoxOnTruck(Player player)
        {
            if (!player.HasData("USER_CLASS")) return;
            Data.Entities.User user = player.GetData<Data.Entities.User>("USER_CLASS");
        ...
        private async static Task<int> GetGananciesOfRace(Player player, Vector3 position)
        {
